    ...
    export default {
        ...
        data() {
            return {
                ...
                blogInfos:[],
                hotTagId:this.$route.params.hotTagId
            }
        },
        mounted() {
            this.$Loading.start()

            this.getBlogInfoByHotTagPage()
        },
        watch:{
            $route(to,from) {

                if (to.path.substr(0, to.path.lastIndexOf("/")) === "/user/blog/hotTag"){
                    this.hotTagId = to.path.substr(to.path.lastIndexOf("/") + 1,to.path.length)
        ...
        methods: {
            getBlogInfoByHotTagPage: function () {
                const that = this
                let data = {
                    "hotTag": parseInt(that.hotTagId),
                ...
                service({
                    method: "post",
                    data: data,
                    url: "/api/blBlog/getBlogInfoByHotTag"
                }).then(res => {
